...
-- Checks DisplayName instead of Name
local function GetPlayer(name)
	name = name:lower()
	for _, p in pairs(game.Players:GetPlayers()) do
		if p.DisplayName:lower():sub(1, #name) == name then
			return p
		end
	end
	return nil
...
local CartTo = function(targetName)
	local target = GetPlayer(targetName)
	if not target then Chat("Player not found") return end
	...
	-- Checks specifically for LocalPlayer.Name.."BIKE"
	local Cart = workspace.OldVehicles:WaitForChild(LocalPlayer.Name.."BIKE", 10) 
	if not Cart then Chat("Cart failed to spawn") return end

	local Seat = Cart:FindFirstChild("Seat") or Cart:FindFirstChild("VehicleSeat")

	for i = 1,5 do
		Seat:Sit(LocalPlayer.Character.Humanoid)
		LocalPlayer.Character:PivotTo(Seat.CFrame)
	end

	Chat("Carting to " .. target.DisplayName)
	
	-- Wait loop to ensure we reach them
	...
	while task.wait() do
		if CommandRunning and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
			if Seat.Occupant ~= LocalPlayer.Character.Humanoid then
				Seat:Sit(LocalPlayer.Character.Humanoid)
			end
			LocalPlayer.Character:PivotTo(target.Character.HumanoidRootPart.CFrame)
		else
...
local CartBring = function(targetName)
	local target = GetPlayer(targetName)
	if not target then Chat("Player not found") return end
	...
	-- Bring to Owner
	while task.wait() do
		if CommandRunning and OwnerInstance.Character and OwnerInstance.Character:FindFirstChild("HumanoidRootPart") then
			if Seat.Occupant ~= LocalPlayer.Character.Humanoid then
				Seat:Sit(LocalPlayer.Character.Humanoid)
			end
			LocalPlayer.Character:PivotTo(OwnerInstance.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,5))
		else
			LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
			break
		end
...
local CartMove = function(speed)
	...
	for i = 1,60 do
		...
		Seat.CFrame = OwnerInstance.Character.HumanoidRootPart.CFrame
		task.wait()
	end

	Instance.new("Highlight",Clone)
	local saved = LocalPlayer.Character.HumanoidRootPart.CFrame*CFrame.new(0,0,-10)
	for i = 1,60 do
		Clone:PivotTo(saved)
		task.wait()
	end
	while task.wait() do
		if CommandRunning then
			-- Ensure sit
			if Seat.Occupant ~= LocalPlayer.Character.Humanoid then Seat:Sit(LocalPlayer.Character.Humanoid) end
...
local Summon = function(otherplayer)
	local target = nil
	if otherplayer then target = GetPlayer(otherplayer) end
	...
	if not target then
		...
		while task.wait() do
			if CommandRunning then
				pcall(function()
					LocalPlayer.Character:PivotTo(target.Character.HumanoidRootPart.CFrame*CFrame.new(0,4,3))
					LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
				end)
			else
				break
...
OwnerInstance.Chatted:Connect(function(msg)
	...
	if msg[1] == ".cartwalk" and msg[2] then
		CommandRunning = true
		CartMove(tonumber(msg[2]))
	end
	if msg[1] == ".cartto" and msg[2] then
		CommandRunning = true
		CartTo(msg[2])
	end
	if msg[1] == ".cartbring" and msg[2] then
		CommandRunning = true
		CartBring(msg[2])
	end
	if msg[1] == "s" then
